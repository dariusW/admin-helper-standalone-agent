...
public class MemoryMonitor extends BaseMonitor {
	...
	private static Logger log = Logger.getLogger(MemoryMonitor.class.getName());
	...
	public JsonData execute() {
		...
		try {
			...
			item.setTable("memory");
			List<String> result;
			if(Integer.parseInt(System.getProperty("sun.arch.data.model"))==64){
				result = execExternalMonitor("agents/MemInfo64.exe");
			} else {
				result = execExternalMonitor("agents/MemInfo.exe");
			}
