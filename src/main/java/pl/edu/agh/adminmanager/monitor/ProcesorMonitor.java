...
@Component
public class ProcesorMonitor extends BaseMonitor {

	private static Logger log = Logger.getLogger(ProcesorMonitor.class.getName());
	...
	public JsonData execute() {
		...
		try {
			...
			item.setTable("procesor");
			List<String> result;
			if(Integer.parseInt(System.getProperty("sun.arch.data.model"))==64){
				result = execExternalMonitor("agents/UsageSummary64.exe");
			} else {
				result = execExternalMonitor("agents/UsageSummary.exe");
			}
